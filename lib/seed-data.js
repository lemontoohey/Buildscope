...
function buildInitialState() {
  ...
  const { buildEstimatorTables } = require('./estimator-seed');
  const estimator = buildEstimatorTables();
  tables.estimate_jobs = estimator.estimate_jobs;
  tables.estimate_line_items = estimator.estimate_line_items;
  ...
  const nextIds = {
    ...
    estimate_jobs: estimator.nextIds.estimate_jobs,
    estimate_line_items: estimator.nextIds.estimate_line_items,
    estimate_attachments: 1,
  };
...
// Same idea as ensureEstimatingState above, for the Job Estimator tables —
// an existing Google Drive data file (saved before this feature existed)
// gets the tables added and demo jobs seeded in place the first time it's
// loaded after upgrading, exactly once.
function ensureEstimatorState(state) {
  if (!state.tables) state.tables = {};
  if (!state.nextIds) state.nextIds = {};
  const needed = ['estimate_jobs', 'estimate_line_items', 'estimate_attachments'];
  for (const table of needed) {
    if (!state.tables[table]) state.tables[table] = [];
    if (state.nextIds[table] === undefined) state.nextIds[table] = 1;
  }
  if (state.tables.estimate_jobs.length === 0) {
    const { buildEstimatorTables } = require('./estimator-seed');
    const estimator = buildEstimatorTables({
      startIds: {
        estimate_jobs: state.nextIds.estimate_jobs || 1,
        estimate_line_items: state.nextIds.estimate_line_items || 1,
      },
    });
    state.tables.estimate_jobs = estimator.estimate_jobs;
    state.tables.estimate_line_items = estimator.estimate_line_items;
    Object.assign(state.nextIds, estimator.nextIds);
  }
  return state;
}
