...
// The whole "database" is one JSON file (owner-build-tracker-data.json) in a
...
function buildGoogleDriveStore() {
  ...
  const FILE_NAME = 'owner-build-tracker-data.json';
  ...
  async function load() {
    ...
    if (!loadingPromise) {
      loadingPromise = (async () => {
        ...
        const text = await res.text();
        const { ensureEstimatingState, ensureEstimatorState } = require('./seed-data');
        ...
        const hadEstimateJobs = parsed.tables && parsed.tables.estimate_jobs && parsed.tables.estimate_jobs.length;
        state = ensureEstimatorState(ensureEstimatingState(parsed));
        if ((!hadSuppliers && state.tables.suppliers.length) || (!hadEstimateJobs && state.tables.estimate_jobs.length)) {
          await persist();
        }
        return state;
      })();
...
// Routes call store.listAll('materials'), store.insert('transactions', ...)
// etc without ever mentioning an account -- this layer is what makes that
// safe with several accounts sharing one physical database. It reads the
// logged-in account id from lib/request-context (set once per request in
// server.js, right after the session cookie is resolved) and:
//   - injects account_id into every insert
//   - adds account_id to every filter-based read (listAll/getWhere/count)
//   - checks a fetched row's account_id before returning it from getById,
//     and before allowing update/remove -- a wrong/guessed id behaves
//     exactly like it doesn't exist, rather than leaking whether it exists
//   - never lets a caller overwrite account_id via update()
//   - prefixes getSetting/setSetting's key with the account id, since the
//     settings table is a plain key/value store with no column to scope
//
// Tables in TENANT_TABLES (see lib/tenant-tables.js) get all of the above.
// Everything else (the shared reference price book, calculator recipes)
// passes straight through untouched, exactly like before multi-tenancy.
const { getAccountId } = require('./request-context');
const { TENANT_TABLES } = require('./tenant-tables');

function requireAccountId() {
  const id = getAccountId();
  if (!id) {
    throw new Error('Internal error: data store accessed outside a signed-in request.');
  }
  return id;
}
...
// startup -- and, for tenant tables, scopes that call to the signed-in
// account first.
    ...
    get(_target, prop) {
      ...
      if (prop === 'listAll') {
        return (table, opts) => {
          ...
          if (!TENANT_TABLES.has(table)) return instance.listAll(table, opts);
          return instance.getWhere(table, { account_id: requireAccountId() }, opts);
        };
      }
      if (prop === 'getWhere') {
        return (table, filters = {}, opts) => {
          const instance = getStoreInstance();
          if (!TENANT_TABLES.has(table)) return instance.getWhere(table, filters, opts);
          return instance.getWhere(table, { ...filters, account_id: requireAccountId() }, opts);
        };
      }
      if (prop === 'getById') {
        return async (table, id) => {
          const instance = getStoreInstance();
          const row = await instance.getById(table, id);
          if (!row) return null;
          if (!TENANT_TABLES.has(table)) return row;
          if (String(row.account_id) !== String(requireAccountId())) return null;
          return row;
        };
      }
      if (prop === 'insert') {
        return (table, data) => {
          const instance = getStoreInstance();
          if (!TENANT_TABLES.has(table)) return instance.insert(table, data);
          return instance.insert(table, { ...data, account_id: requireAccountId() });
        };
      }
      if (prop === 'update') {
        return async (table, id, data) => {
          const instance = getStoreInstance();
          if (!TENANT_TABLES.has(table)) return instance.update(table, id, data);
          const existing = await instance.getById(table, id);
          if (!existing || String(existing.account_id) !== String(requireAccountId())) return null;
          const { account_id, ...safeData } = data; // never let a caller move a row to another account
          return instance.update(table, id, safeData);
        };
      }
      if (prop === 'remove') {
        return async (table, id) => {
          const instance = getStoreInstance();
          if (!TENANT_TABLES.has(table)) return instance.remove(table, id);
          const existing = await instance.getById(table, id);
          if (!existing || String(existing.account_id) !== String(requireAccountId())) return false;
          return instance.remove(table, id);
        };
      }
      if (prop === 'count') {
        return (table, filters = {}) => {
          const instance = getStoreInstance();
          if (!TENANT_TABLES.has(table)) return instance.count(table, filters);
          return instance.count(table, { ...filters, account_id: requireAccountId() });
        };
      }
      if (prop === 'getSetting') {
        return (key) => getStoreInstance().getSetting(`${requireAccountId()}:${key}`);
      }
      if (prop === 'setSetting') {
        return (key, value) => getStoreInstance().setSetting(`${requireAccountId()}:${key}`, value);
      }
