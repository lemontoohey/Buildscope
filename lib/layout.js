  ...
  <style>
    ...
    */
    :root {
      ...
      --violet-glow: #8b5cf6; /* Camille_website uColorGlow -- used at near-zero opacity only */
      --pistachio-card: rgba(221, 228, 207, 0.88); /* pale pistachio, pre-mixed with ~5% --violet-3 , alpha < 1 so the shader shows through */
      --pistachio-border: #8aa06b; /* a shade darker/greener than the card surface, for box + header outlines */
    ...
    .border-slate-200, .border-slate-300 {
      border-color: var(--pistachio-border) !important;
    }
...
<body class="bg-[#f9f9f9] text-[${COLORS.bodyText}] min-h-screen">
  ...
  <svg id="bg-planoutline" aria-hidden="true" viewBox="0 0 1000 863" preserveAspectRatio="xMidYMid slice"
    ...
    <g fill="none" stroke="#5a4a6e" stroke-width="1.4" stroke-linejoin="round" stroke-linecap="round" opacity="0.06">
  ...
  <script>
    (function () {
      ...
      var LAYERS = [
        { count: 14, minR: 1,   maxR: 2,   drift: 0.010, parallax: 0.02, color: 'rgba(90,74,110,0.10)' },
        { count: 10, minR: 2,   maxR: 3.5, drift: 0.018, parallax: 0.05, color: 'rgba(74,54,88,0.10)' },
        { count: 6,  minR: 3,   maxR: 5,   drift: 0.028, parallax: 0.09, color: 'rgba(54,37,72,0.09)' },
      ];
