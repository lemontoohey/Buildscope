...
<head>
  ...
  <title>${escapeHtml(title)} — Build Tracker</title>
  ...
  <style>
    ...
    /*
      ...
      outline, and that same brown used as an imperceptibly faint,
      macOS-style soft drop shadow. Nothing here is meant to visually
      *read* as violet or brown at a glance -- it's mixed in under a
      pale pistachio surface so the surface still looks like a plain
      white-ish card, just warmer and quietly tied to the same palette
      as the background shader underneath it.
    */
    :root {
      ...
      --void: #06000c; /* Camille_website's own --color-void -- the page canvas itself, not just a background tint */
      --paper-dark: #0c0614; /* Camille_website's uColorPaper -- one notch lighter than --void, for the header bar to sit slightly apart from the page */
      --parchment: #fdf5e6; /* Camille_website's --color-parchment -- their one colour for all typography on a dark ground */
      --pistachio-card: rgba(221, 228, 207, 0.88); /* pale pistachio, pre-mixed with ~5% --violet-3 , alpha < 1 so the shader shows through -- cards stay light, floating on the dark canvas */
      --card-border: #a4a5a2; /* near-neutral grey, mixed 90% grey / 7% pistachio-green / 3% violet -- close enough to plain grey that the tint isn't meant to be consciously noticed */
      --brown-halo: 150, 40, 20; /* Camille_website "benzi" Benzimidazolone brown, as an r,g,b triplet for rgba() */
    }
    ...
    .bg-white {
      background-color: var(--pistachio-card) !important;
      color: #484848 !important; /* cards stay light, so their own text stays dark -- see the <main>/nav rules below for why this needs restating */
      box-shadow:
    ...
    /*
      The page canvas itself is now Camille_website's dark violet, not a
      pale grey -- so any text sitting directly on it (page titles,
      section headings) needs to flip to their light parchment colour by
      default. Cards are the exception: they're deliberately still pale
      pistachio surfaces "floating" on that dark canvas (recolouring every
      table/label across the app to work in a dark card would be a much
      bigger job than asked for here), so .bg-white resets the text colour
      straight back to dark as soon as you're inside one -- that's what
      the rule above is for. Nothing in the markup had to change for
      this: it's just two rules of different specificity racing each
      other, and the more specific one (.bg-white, a class selector) always
      wins over the plainer one (main, a bare element selector).
    */
    main {
      color: var(--parchment);
    }
    ...
    /*
      ...
      element that renders dark instead of pistachio.
    */
    nav.bg-white {
...
<body class="bg-[#06000c] text-[${COLORS.bodyText}] min-h-screen">
  ...
  <svg id="bg-planoutline" aria-hidden="true" viewBox="0 0 1000 863" preserveAspectRatio="xMidYMid slice"
    ...
    <g fill="none" stroke="#8a7aa8" stroke-width="1.4" stroke-linejoin="round" stroke-linecap="round" opacity="0.14">
    ...
    <div class="${shellWidth} mx-auto px-4">
      <div class="flex items-center justify-between h-20 gap-4">
        <div class="flex items-center">
          <span class="brand-mark font-black text-2xl uppercase tracking-wide text-[${COLORS.brandBlue}]">Build Tracker</span>
        </div>
  ...
  <script>
    (function () {
      ...
      var LAYERS = [
        { count: 14, minR: 1,   maxR: 2,   drift: 0.010, parallax: 0.02, color: 'rgba(90,74,110,0.45)' },
        { count: 10, minR: 2,   maxR: 3.5, drift: 0.018, parallax: 0.05, color: 'rgba(122,88,168,0.5)' },
        { count: 6,  minR: 3,   maxR: 5,   drift: 0.028, parallax: 0.09, color: 'rgba(139,92,246,0.4)' },
      ];
