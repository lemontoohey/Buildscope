...
function takeoffFields(categoryNames) {
  return {
    ...
    unusual_features: {
      type: 'array',
      description:
        'Do this pass FIRST, before drafting items below: scan every page for anything you notice that ' +
        "is NOT a standard, repeatable residential element — a curved or non-rectangular wall, an unusual " +
        'roofline or structural detail, bespoke joinery, an odd-shaped room, a one-off feature with no ' +
        "normal unit-rate equivalent. List each in plain language, even if you can't quantify it or fit it " +
        'into a budget category below. The point is to surface anything unusual rather than silently ' +
        'dropping it because it did not fit a normal line item.',
      items: { type: 'string' },
    },
    items: {
      type: 'array',
      ...
      items: {
        ...
        properties: {
          ...
          confidence: {
            type: 'string',
            enum: ['high', 'medium', 'low'],
            description: 'How clearly the drawings let you read/derive this quantity — not whether the element itself is unusual (see is_novel for that).',
          },
          is_novel: {
            type: 'boolean',
            description:
              'true if this is a bespoke/non-standard element (not something you would price with a normal ' +
              "unit rate because it doesn't repeat across typical homes) — e.g. a curved feature wall, an " +
              'unusual structural member, custom joinery. false for ordinary repeatable elements (standard ' +
              'stud wall, a normal window, a typical slab area) even if you are only medium/low confidence ' +
              'about the exact quantity.',
          },
          novelty_reason: {
            type: 'string',
            description: 'One short phrase on why this is non-standard, only when is_novel is true. Empty string otherwise.',
          },
        },
...
const TAKEOFF_PROMPT =
  ...
  "owner-build. Draft a STARTING bill of quantities via the draft_takeoff tool/function. Work in two " +
  'passes: FIRST, scan every page for anything unusual — a curved or non-rectangular wall, an odd roofline, ' +
  'bespoke joinery, a structural detail you would not see on a standard project-home plan — and record each ' +
  'in unusual_features even if you cannot quantify it or fit it into a normal item below (this pass matters: ' +
  'a generic takeoff tool tends to silently drop whatever does not match a standard element, which is ' +
  'exactly the failure mode this list exists to avoid). SECOND, draft the items: total internal floor area ' +
  'and total roof area in m2 if the drawings let you estimate them (state your method in assumptions); a ' +
  'full glazing/joinery list — every window and door tagged on a window/door schedule (codes like W1.01, ' +
  'D01) with its size/type; and rough quantities for big-ticket categories where the drawings support it ' +
  '(concrete volume for slab/footings as area x an assumed depth — state the assumed depth in assumptions; ' +
  'roof sheeting area; insulation area). For every item, set is_novel true if it is a bespoke one-off rather ' +
  'than something a normal unit rate would price correctly — this is a different judgement from confidence ' +
  '(confidence is about how clearly you could read the drawing; is_novel is about whether a standard rate ' +
  'even applies). This is a starting checklist for an owner-builder to confirm against a real quote or ' +
  "quantity surveyor, not a substitute for one — if you can't make out a schedule or measurement, omit it " +
  'rather than inventing a number. Under-confidence is safer than over-confidence, and flagging something as ' +
  'novel is safer than pricing it like a routine item.';
...
const REVIEW_PROMPT =
  ...
  'a placeholder or an obviously round/default number nobody has actually checked. Also specifically look ' +
  'for the "generic rate on a custom job" trap: a materials line whose description sounds bespoke or ' +
  'one-off (curved, custom, feature, bespoke, non-standard, or an unusual shape/size) priced at a suspiciously ' +
  'round number, or a category that reads as if a blended $/m2-style rate was applied rather than a real, ' +
  'itemised quote — flag these explicitly as "this looks like a generic rate on what may be a bespoke item — ' +
  'get an actual quote before relying on this number". Do NOT flag things that are simply early / not ' +
  'started yet in a normal way — only flag things that look genuinely overlooked or genuinely suspect. Be ' +
...
// --- AI quote/invoice import -------------------------------------------
// Same shape as the plan takeoff above: page images in, a draft set of
// priced line items out. Nothing is written to the database here — the
// Materials "Import a quote" screen shows every line for review/editing
// before anything is saved, same as the plan takeoff and receipt flows.

function quoteImportFields(categoryNames) {
  return {
    supplier: { type: 'string', description: 'The business that issued this quote/invoice, if shown.' },
    quote_date: { type: 'string', description: 'Date on the document, as YYYY-MM-DD if determinable. Omit if not shown.' },
    line_items: {
      type: 'array',
      description: 'Every priced line on the document.',
      items: {
        type: 'object',
        properties: {
          description: { type: 'string', description: 'The line item as described on the document.' },
          category: {
            type: 'string',
            description: `Best-guess budget category, chosen from exactly this list: ${categoryNames.join(', ')}.`,
          },
          quantity: { type: 'number', description: 'Quantity if shown, otherwise 1.' },
          unit: { type: 'string', description: 'e.g. m2, m3, ea, lm, hr.' },
          unit_price: { type: 'number', description: 'Price per unit in dollars, if the document states a rate. Omit if only a line total is given.' },
          line_total: { type: 'number', description: 'Total price for this line in dollars, as shown or as quantity x unit_price.' },
          generic_rate_suspected: {
            type: 'boolean',
            description:
              'true if this line describes something bespoke/custom/one-off (curved, feature, custom, non-standard) but is priced at a suspiciously round, blended-looking rate rather than an itemised figure -- the "generic rate on a custom job" trap.',
          },
          generic_rate_reason: {
            type: 'string',
            description: 'One short phrase why, only when generic_rate_suspected is true. Empty string otherwise.',
          },
        },
        required: ['description', 'category', 'quantity', 'line_total'],
      },
    },
  };
}

const QUOTE_IMPORT_PROMPT =
  'These are page images of a supplier quote or invoice for a residential owner-build. Extract every ' +
  'priced line item via the extract_quote tool/function: its description as written, a best-guess budget ' +
  'category from the given list, quantity and unit if shown, and the unit price and/or line total in ' +
  'dollars. If the document only gives a lump sum for the whole job with no itemised breakdown, record it ' +
  'as a single line item describing the overall scope. Note the supplier name and document date if shown. ' +
  'For each line, flag generic_rate_suspected true if the item sounds bespoke or custom (curved, feature, ' +
  'non-standard, unusual shape/size) but is priced at a suspiciously round or blended-looking rate rather ' +
  'than a real itemised figure -- this is exactly the "generic rate on a custom job" trap owner-builders ' +
  'get caught by. Be literal -- transcribe what the document says rather than estimating; if a number is ' +
  'not legible, omit that field rather than inventing one.';

async function callAnthropicQuoteImport({ apiKey, pages, categoryNames }) {
  const model = process.env.ANTHROPIC_MODEL || 'claude-sonnet-5';
  const tool = {
    name: 'extract_quote',
    description: 'Records line items extracted from a supplier quote or invoice document.',
    input_schema: { type: 'object', properties: quoteImportFields(categoryNames), required: ['line_items'] },
  };
  const imageBlocks = pages.map((p) => ({
    type: 'image',
    source: { type: 'base64', media_type: p.mediaType || 'image/png', data: p.base64 },
  ...
  const response = await fetch('https://api.anthropic.com/v1/messages', {
    ...
    body: JSON.stringify({
      ...
      messages: [{ role: 'user', content: [...imageBlocks, { type: 'text', text: QUOTE_IMPORT_PROMPT }] }],
    }),
  });
  if (!response.ok) {
    const errText = await response.text().catch(() => '');
  ...
  const data = await response.json();
  const toolUse = (data.content || []).find((block) => block.type === 'tool_use');
  if (!toolUse) throw new Error('Claude did not return structured quote line items for these pages.');
  return toolUse.input;
}

async function callOpenAiQuoteImport({ apiKey, pages, categoryNames }) {
  const model = process.env.OPENAI_MODEL || 'gpt-4o';
  const tool = {
    type: 'function',
    function: {
      name: 'extract_quote',
      description: 'Records line items extracted from a supplier quote or invoice document.',
      parameters: { type: 'object', properties: quoteImportFields(categoryNames), required: ['line_items'] },
    },
  };
  const imageParts = pages.map((p) => ({
    type: 'image_url',
  ...
  const response = await fetch('https://api.openai.com/v1/chat/completions', {
    ...
    body: JSON.stringify({
      ...
      messages: [{ role: 'user', content: [{ type: 'text', text: QUOTE_IMPORT_PROMPT }, ...imageParts] }],
    }),
  });
  if (!response.ok) {
    const errText = await response.text().catch(() => '');
  ...
  const data = await response.json();
  const call = data.choices?.[0]?.message?.tool_calls?.[0];
  if (!call) throw new Error('OpenAI did not return structured quote line items for these pages.');
  return JSON.parse(call.function.arguments);
}

async function extractQuoteLines({ pages, categoryNames }) {
  const { provider, key } = await getAiConfig();
  if (!key) {
    throw new Error('No AI provider configured -- add an API key on the Settings page to import quotes automatically.');
  }
  if (!pages || !pages.length) {
    throw new Error('No pages to read.');
  }
  if (provider === 'openai') return callOpenAiQuoteImport({ apiKey: key, pages, categoryNames });
  return callAnthropicQuoteImport({ apiKey: key, pages, categoryNames });
}
