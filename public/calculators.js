...
//
// This file is the single source of truth for that arithmetic, and it's
// written to run in two places unchanged:
//   - required by Node on the server (routes/calculators.js, via
//     lib/calculators.js) -- kept only so the page still renders something
//     sane if JS is disabled, or as a reference table.
//   - loaded as a plain <script> in the browser (routes/calculators.js
//     includes it directly), where it does the actual computing. Because
//     it's a flat static file under /public/, the service worker caches it
//     like any other static asset, and because the maths never leaves the
//     browser, the calculators page works with zero signal once the page
//     itself has been opened once while online.
//
...
// pure function it calls. Adding a seventh calculator later is a data-table
// entry, not new UI code.
...
function unitsForArea(opts) {
  var areaM2 = opts.areaM2, coveragePerUnitM2 = opts.coveragePerUnitM2, wastagePct = opts.wastagePct || 0;
...
function volumeM3(opts) {
  var lengthM = opts.lengthM, widthM = opts.widthM, depthM = opts.depthM, wastagePct = opts.wastagePct || 0;
...
function unitsForSpacing(opts) {
  var runLengthM = opts.runLengthM, spacingMm = opts.spacingMm;
  if (!runLengthM || !spacingMm) return 0;
  return roundUp((runLengthM * 1000) / spacingMm) + 1;
}

// Paint: area x coats, at a coverage rate per litre.
function paintLitres(opts) {
  var areaM2 = opts.areaM2, coats = opts.coats || 2, coveragePerLitreM2 = opts.coveragePerLitreM2, wastagePct = opts.wastagePct || 0;
...
function compute(calc, values) {
  var num = function (v) { return v === undefined || v === null || v === '' ? 0 : parseFloat(v); };
  switch (calc.kind) {
    case 'area':
      return unitsForArea({
...
var CalculatorsApi = {
  ...
  compute: compute,
  unitsForArea: unitsForArea,
  volumeM3: volumeM3,
  unitsForSpacing: unitsForSpacing,
  paintLitres: paintLitres,
};

if (typeof module !== 'undefined' && module.exports) {
  module.exports = CalculatorsApi;
} else {
  window.Calculators = CalculatorsApi;
}
